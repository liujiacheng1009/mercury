...
#include "double_sphere_camera.hpp"
#include "extended_camera.hpp"
#include "fov_camera.hpp"
...
public:
  ...
  inline GenericCamera<Scalar2> cast() const
  {
    ...
    std::visit([&](const auto &v)
               { res.variant = v.template cast<Scalar2>(); },
               variant);
    return res;
  }
  ...
  {
    std::visit([&](auto &v)
               { return v.setFromInit(init); }, variant);
  }
                      ...
                      Mat24 *d_proj_d_p3d = nullptr) const
  {
    ...
    std::visit(
        [&](const auto &cam)
        { res = cam.project(p3d, proj, d_proj_d_p3d); },
        variant);
    return res;
  }
                        ...
                        Mat42 *d_p3d_d_proj = nullptr) const
  {
    ...
    std::visit(
        [&](const auto &cam)
        { res = cam.unproject(proj, p3d, d_p3d_d_proj); },
        variant);
    return res;
  }
  ...
  inline void project(const AlignedVector<Vec3> &p3d, const Mat4 &T_c_w,
                      AlignedVector<Vec2> &proj,
  ...
  /// @param[out] proj results of projection
  /// @param[out] proj_success if projection is valid
  inline void project(const AlignedVector<Vec4> &p3d, const Mat4 &T_c_w,
                      AlignedVector<Vec2> &proj,
                      std::vector<bool> &proj_success) const
  {
    std::visit(
  ...
  /// @param[out] proj results of projection
  /// @param[out] proj_success if projection is valid
  inline void project(const AlignedVector<Vec4> &p3d,
                      AlignedVector<Vec2> &proj,
                      std::vector<bool> &proj_success) const
  {
    std::visit(
  ...
  /// @param[out] proj_success if projection is valid
  inline void project(
      const AlignedVector<Vec4> &p3d, const Mat4 &T_c_w,
      AlignedVector<Vec2> &proj, std::vector<bool> &proj_success,
      AlignedVector<Vec2> &polar_azimuthal_angle) const
  {
    std::visit(
        [&](const auto &cam)
        {
  ...
  inline void unproject(const AlignedVector<Vec2> &proj,
                        AlignedVector<Vec4> &p3d,
