...
TEST(PreIntegrationTestSuite, RelPoseTest) {
  ...
  relPoseError(T_i_j, T_w_i, T_w_j, &d_res_d_T_w_i, &d_res_d_T_w_j);

  {
    ...
    test_jacobian(
        ...
        [&](const Sophus::Vector6d& x) {
          ...
          PoseState<double>::incPose(x, T_w_i_new);

          return relPoseError(T_i_j, T_w_i_new, T_w_j);
        },
        x0);
  }

  {
    ...
    test_jacobian(
        ...
        [&](const Sophus::Vector6d& x) {
          ...
          PoseState<double>::incPose(x, T_w_j_new);

          return relPoseError(T_i_j, T_w_i, T_w_j_new);
        },
        x0);
  }
}
...
TEST(PreIntegrationTestSuite, AbsPositionTest) {
  ...
  absPositionError(T_w_i, pos, &d_res_d_T_w_i);

  {
    ...
    test_jacobian(
        ...
        [&](const Sophus::Vector6d& x) {
          ...
          return absPositionError(T_w_i_new, pos);
        },
        x0);
  }
}
...
TEST(PreIntegrationTestSuite, YawTest) {
  ...
  yawError(T_w_i, yaw_dir_body, &d_res_d_T_w_i);

  {
    ...
    test_jacobian(
        ...
        [&](const Sophus::Vector6d& x) {
          ...
          double res = yawError(T_w_i_new, yaw_dir_body);
...
TEST(PreIntegrationTestSuite, RollPitchTest) {
  ...
  rollPitchError(T_w_i, R_w_i, &d_res_d_T_w_i);

  {
    ...
    test_jacobian(
        ...
        [&](const Sophus::Vector6d& x) {
          ...
          return rollPitchError(T_w_i_new, R_w_i);
        },
        x0);
  }
}
